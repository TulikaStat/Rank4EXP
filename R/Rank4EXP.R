...
Rank4EXP<-function(x1,x2,x3,x4,alpha){
  ...
  phi_n_function<-function(x1,x2,x3,x4) {
    ...
    for (i in 1:nsim[4]) {
      R_4[1,i]<-ui_14[nsim[1]+i]
    }
    for (i in 1:nsim[4]) {
      R_4[2,i]<-ui_24[i+nsim[2]]
    }
    for (i in 1:nsim[4]) {
    ...
    w_14<-w(nsim[1],R_3_bar[1],nsim[4])
    w_24<-w(nsim[2],R_3_bar[2],nsim[4])
    w_34<-w(nsim[3],R_3_bar[3],nsim[4])
    w_44<-w(nsim[4],R_3_bar[4],nsim[4])

    wmatrix<-matrix(c(w_11,w_21, w_31,w_41,w_12, w_22, w_32,w_42,w_13, w_23, w_33,w_43, w_14,w_24, w_34, w_44), nrow = k, ncol = k)
    wmatrix
    ##################################### p_hat
    p_hat<-rep(0,k)
    for (i in 1:k) {
    ...
    G_function<-function(x){
      (G1_function(x)+G2_function(x)+G3_function(x)+G4_function(x))/k
    }
  ...
  phi_n<-phi_n_function(x1,x2,x3,x4)$statistic_a
  N<-sum(nsim)
  theta_boot<-((nsim[1]*mean(x1))+(nsim[2]*mean(x2))+(nsim[3]*mean(x3))+(nsim[4]*mean(x4)))/N
  phi_n_boot<-rep(0,bl)
  for (boot in 1:bl) {
    ...
    phi_n_boot[boot]<-phi_n_function(x1_boot,x2_boot,x3_boot,x4_boot)$statistic_a
  }
  new_Q<- phi_n_boot[order(phi_n_boot)]
  C_star_Q<- new_Q[floor((1-alpha)*bl)]


  phintilde<-  phi_n_function(x1,x2,x3,x4)$statistic_b

  phintildecv<-qchisq(1-alpha, df=phi_n_function(x1,x2,x3,x4)$f_hat, lower.tail=TRUE)

  phin<-phi_n_function(x1,x2,x3,x4)$statistic_c
  phincv<-phi_n_function(x1,x2,x3,x4)$df_hat
